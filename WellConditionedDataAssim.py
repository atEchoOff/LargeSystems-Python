...
def solve(θ, n, m, Δt, nt, mt, W, K, H, z, R, ρ):
    '''
    Run the well-conditioned data assim. problem outlined in 2.116
    ...
    # Add a bunch of zeros to the bottom of z
    z = np.vstack((z, np.zeros((n, 1), dtype=np.float64)))

    def A(v):
        # First define how to multiply matrix A
        # Algorithm outlined on page 86 to 87
        # Modified to include sqrt(ρ)R at the bottom of the matrix
        ...
        # Add sqrt(ρ)R to the bottom of the matrix
        return np.vstack((ret, np.sqrt(ρ) * R * v))
    
    def At(w):
        # Define how to multiply matrix At
        # Algorithm outlined on page 87
        # Modified to add sqrt(ρ)Rᵀ to the end

        v = np.zeros((n, 1), dtype=np.float64)

        # First, scale by weight matrix
        w[0:m * mt] = W.T * w[0:m * mt]
        ...
        # Add the final √ρ * Rᵀ to the end
        return v + np.sqrt(ρ) * R.T * w[m * mt:w.shape[0]]
